...
# 文件 scripts/src/core/data/data_class.py 定义了一些用于处理代谢物数据的类和函数。以下是该文件的主要功能
# 处理输入代谢物的数据，包括碳同位素比例和丰度列表。
# 提供生成中间代谢物分布向量的方法
# 这些类和函数的主要目的是处理和分析代谢物数据，特别是用于代谢流分析（MFA）。
...
# 从多个代谢流分析数据对象中提取实验数据对象，然后对这些实验数据对象进行平均。
# 平均多个代谢流分析数据。
