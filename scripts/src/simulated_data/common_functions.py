...
# scripts/src/simulated_data/common_functions.py
# 是一个用于生成模拟代谢物同位素分布(MID)数据的工具模块

class Keyword(Keywords):
    # 定义关键字常量，用于数据处理和文件输出
    metabolite_key_name = 'metabolite_key_name' # 代谢物关键名
    raw_data_vector = 'raw_data_vector' # 原始数据向量
    raw_data_vector_str = 'raw_data_vector_str' # 原始数据向量字符串
    standard_metabolite_name = 'standard_metabolite_name'   # 标准代谢物名称
    combined_metabolite_list = 'combined_metabolite_list'   # 组合代谢物列表
    combined_metabolite_list_str = 'combined_metabolite_list_str'
    mixed_compartment_list = 'mixed_compartment_list'   # 混合区室列表
    mixed_compartment_list_str = 'mixed_compartment_list_str'

    normal_random = 'normal'    # 正态随机分布
    uniform_random = 'uniform'  # 均匀随机分布
...
def add_noise_to_mid_data_obj(raw_mid_data_obj_dict, noise_sigma=0.1):
    """
        向MID数据对象添加噪声
        Args:
            raw_mid_data_obj_dict: 原始MID数据对象字典
            noise_sigma: 噪声标准差
        Returns:
            添加噪声后的MID数据对象字典
    """
...
def add_noise_to_mid_data_vector(raw_mid_data_vector_dict, noise_factor=0.1, random_func=Keyword.normal_random):
    """
        向MID数据向量添加噪声
        Args:
            raw_mid_data_vector_dict: 原始数据向量字典
            noise_factor: 噪声因子
            random_func: 随机函数类型
        Returns:
            添加噪声后的数据向量字典
...
def simulated_mid_data_generator(
    ...
    """
        主要的MID数据生成函数
        Args:
            new_optimization_flux: 是否生成新的优化通量
            simulated_data_batch_num: 模拟数据批次数量
            index: 数据索引
            with_noise: 是否添加噪声
            with_glns_m: 是否包含GLNS_m通量
    """
